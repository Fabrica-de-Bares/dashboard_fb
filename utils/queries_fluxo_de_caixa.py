...
def GET_DETALHES_DESPESAS():
  return dataframe_query(f'''
    ...
    SELECT
      ...
      CASE
        WHEN (tdp.PARCELA_PAGA = 1) AND (tdp.FK_STATUS_PGTO IN (103, 107)) THEN 'Pago'
        WHEN (tdp.PARCELA_PAGA IS NULL OR tdp.PARCELA_PAGA = 0) AND (tdp.FK_STATUS_PGTO IN (100, 108)) THEN 'Pendente'                 
        # ELSE 'Pendente'
      END as 'Status_Pgto'
    FROM T_DESPESA_RAPIDA tdr 
