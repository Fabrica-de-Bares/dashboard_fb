...
def GET_DESPESAS_PENDENTES():
  return dataframe_query(f'''
    ...
    AND tdr.BIT_CANCELADA = 0
    AND (tdr.FK_APROVACAO_DIRETORIA IS NULL OR tdr.FK_APROVACAO_DIRETORIA IN (100, 101, 103, 105, 108))
    AND (tdp.FK_STATUS_PGTO IS NULL OR tdp.FK_STATUS_PGTO NOT IN (103, 107))
''')
...
def GET_DESPESAS_PAGAS():
  return dataframe_query(f'''
    ...
    AND tdr.BIT_CANCELADA = 0
    AND (tdr.FK_APROVACAO_DIRETORIA IS NULL OR tdr.FK_APROVACAO_DIRETORIA IN (100, 101, 103, 105, 108))
    AND tdp.FK_STATUS_PGTO IN (103, 107)
''')
...
def GET_DETALHES_DESPESAS():
  return dataframe_query(f'''
    ...
    SELECT
      ...
      CASE
        WHEN tdp.FK_STATUS_PGTO IN (103, 107) THEN 'Pago'
        # WHEN (tdp.PARCELA_PAGA IS NULL OR tdp.PARCELA_PAGA = 0) AND (tdp.FK_STATUS_PGTO NOT IN (103, 107)) THEN 'Pendente'                 
        ELSE 'Pendente'
      END as 'Status_Pgto'
    FROM T_DESPESA_RAPIDA tdr 
    ...
    AND tdr.BIT_CANCELADA = 0
    AND (tdr.FK_APROVACAO_DIRETORIA IS NULL OR tdr.FK_APROVACAO_DIRETORIA IN (100, 101, 103, 105, 108))
    AND (tdp.PARCELA_PAGA IS NULL OR tdp.PARCELA_PAGA = 0 OR tdp.PARCELA_PAGA = 1 OR tdp.FK_STATUS_PGTO IN (100, 103, 107, 108))                     
''')
...
def GET_DESPESAS_FORA_PRAZO():
  ...
  SELECT 
    ...
    tccg.DESCRICAO AS 'Class. Cont. 1',
    ...
    DATEDIFF(tdr.VENCIMENTO, tdr.LANCAMENTO) AS 'Dias de Antecedência'
  FROM T_DESPESA_RAPIDA tdr
  INNER JOIN T_EMPRESAS te ON te.ID = tdr.FK_LOJA
  INNER JOIN T_FORNECEDOR tf ON tf.ID = tdr.FK_FORNECEDOR
  LEFT JOIN T_CLASSIFICACAO_CONTABIL_GRUPO_1 tccg ON tccg.ID = tdr.FK_CLASSIFICACAO_CONTABIL_GRUPO_1 
  LEFT JOIN T_CLASSIFICACAO_CONTABIL_GRUPO_2 tccg2 ON tccg2.ID = tdr.FK_CLASSIFICACAO_CONTABIL_GRUPO_2 
  WHERE DATEDIFF(tdr.VENCIMENTO, tdr.LANCAMENTO) < 15
